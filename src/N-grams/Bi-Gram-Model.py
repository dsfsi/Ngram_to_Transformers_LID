...
def create_n_grams(lang, num, k): #returns top k n-grams according to frequency
    # new_sents = []
    # for sent in lang:
    #         clean_t = clean_text(sent)
    #         if clean_t != '':
    #                  new_sents.append(clean_t)
    # lang = " ".join(new_sents)
    lang = " ".join(lang) 
...
def language_identify_evaluate(file_address, st, model): # argument 'st' denotes whether you are uploading a file or directly copying text
    test_bigrams = []
    test_trigrams = []
    test_quadgrams = []
    test_file = []
    ...
    # test_trigrams = create_n_grams(file_address, 3, k)
    # test_quadgrams = create_n_grams(file_address, 4, k)
    bi_dist = matching_score_2(test_bigrams, model, 2) 
    # tri_dist = matching_score_2(test_trigrams, tri_grams, 3)
    # quad_dist = matching_score_2(test_quadgrams, quad_grams, 4) 
    #print (bi_dist, tri_dist)
    final_dist = {}
...
# Model evaluate on devtest
def evaluate(path_to_data, models):
                    ...
                    if os.path.isdir(sub_item_path):
                        ...
                        if  files_manager['test_file']:    
                                    ...
                                    for line in corpus:
                                                 ...
                                                 if line != "":
                                                        l_identified = language_identify_evaluate(line, 'string', models)
                                                        predictions.append(l_identified)
                                                        true_values.append(language)
                                                        if language == l_identified:
                                                                    evals[language][0] +=1
                                    evals[language][1] = len(corpus)
                        ...
                        if  files_manager['dev_file']:    
                                    ...
                                    for line in corpus:
                                                 l_identified = language_identify_evaluate(line, 'string', models)
                                                 if language == l_identified:
                                                            dev_evals[language][0] +=1
                                    dev_evals[language][1] = len(corpus)
        return evals
...
models_root_path = './Bi-Models/'
bi_sorted        = "../../../../../ext_data/thapelo/CTEXT_Work/AlignedGov/Bi-Gram-Alinged/"
...
if __name__ == "__main__":
        ...
        for item in os.listdir(train_data_root_path ):
            ...
            if os.path.isdir(item_path):
                print("-------------------------------------------------------------")
                print("Generating Bi-gram For language: ", item_path.split("/")[-1])
                lang_list.append(item_path.split("/")[-1])
                # Loop through the contents of the subfolder
                for sub_item in os.listdir(item_path):
                    # Construct the full path to the sub-item
                    sub_item_path = os.path.join(item_path, sub_item)
                    ...
                    # Check if the sub-item is a file or directory
                    if os.path.isdir(sub_item_path):
                        language = item_path.split("/")[-1].strip()
                        files_manager = {}
                        for file_name in os.listdir(sub_item_path):
                                file_path = os.path.join(sub_item_path, file_name)
        ...
        save_model(bi_grams,  models_root_path +'bigram_model_clean_data.pkl')
        # Evaluate on devtest
        n_grams = {2 : bi_grams}
        ...
        evaluation_scores = evaluate(train_data_root_path, bi_grams)
        with open('./N-Gram_Results-clean-dataset/Bi-gram-results/N_2_test_gram_evaluation_doc_clean_data.txt', 'w') as e_file:
        ...
        with open('./N-Gram_Results-clean-dataset/Bi-gram-results/N_2_dev_gram_evaluation_doc_clean_data.txt', 'w') as e_file:
                                #    e_file.write(f'For {item} : model correclty predicted {values[1]/values[1]*100}% of {values[0]} sentences \n')
                                e_file.write(str(list(dev_evals.keys())) + ' ||| ' + str([values[0]/values[1]*100 for values in list(dev_evals.values())])) 
        fname = "N-Gram_Results-clean-dataset/Bi-gram-results/conf_bigram_clean.jpeg" 
        fname_pdf = "N-Gram_Results-clean-dataset/Bi-gram-results/"
        metric_scores = evaluation(true_values, predictions, label2id, fname) 
                # Save results
        output_test_results_file = "./N-Gram_Results-clean-dataset/Bi-gram-results/bi_gram_results_clean_text.txt"
        ...
        #                                         with open(bi_sorted + 'bi_gram_filtered_nso.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')
        #                                 elif  identified_language == 'tsn':
        #                                     with open(bi_sorted + 'bi_gram_filtered_tsn.txt', 'a') as tsn_file:
        #                                                         tsn_file.write(sentence + '\n') 
        #                                 elif identified_language == "sot":
        #                                     with open(bi_sorted + 'bi_gram_filtered_sot.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')
        #                                 elif identified_language == 'xho':
        #                                         with open(bi_sorted + 'bi_gram_filtered_xho.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')   
        #                                 elif identified_language == 'zul':
        #                                     with open(bi_sorted + 'bi_gram_filtered_zul.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')   
        #                                 elif identified_language == "ssw":
        #                                     with open(bi_sorted + 'bi_gram_filtered_ssw.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')   
        #                                 elif identified_language == "ven":
        #                                     with open(bi_sorted + 'bi_gram_filtered_ven.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n') 
        #                                 elif identified_language == 'tso':
        #                                         with open(bi_sorted + 'bi_gram_filtered_tso.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')   
        #                                 elif identified_language == 'af':
        #                                         with open(bi_sorted + 'bi_gram_filtered_af.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')                                                                                                                                   
        #                                 elif identified_language == 'nbl':
        #                                     with open(bi_sorted + 'bi_gram_filtered_nbl.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')
        #                                 elif identified_language == 'eng':
        #                                     with open(bi_sorted + 'bi_gram_filtered_en.txt', 'a') as n_file:
        #                                                         n_file.write(sentence + '\n')  
