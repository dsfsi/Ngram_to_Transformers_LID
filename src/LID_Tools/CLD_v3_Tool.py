...
def get_cldlid_prediction(text):
#   predictions = cl.classify(text, max_outputs=1)
  text = text.replace("\n"," ")
  predictions = cld3.get_language(text)
  return predictions.language, predictions.probability
...
if __name__ == "__main__":
        ...
        df_test_data['predict_iso'], df_test_data['predict_score'] = zip(*df_test_data['sentence'].progress_apply(get_cldlid_prediction))
        # df_test_data.head()

        df_test_data.to_csv('./CLDLID_Output_CSV_' + str(args.seed)+'.csv', index=False)

        clean_df['predict_iso'], clean_df['predict_score'] = zip(*clean_df['sentence'].progress_apply(get_cldlid_prediction))
        # df_test_data.head()

        clean_df.to_csv('./CLDLID_Output_CSV_Clean_Text_' + str(args.seed)+'.csv', index=False)
