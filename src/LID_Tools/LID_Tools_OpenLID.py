...
model_path = hf_hub_download(repo_id="laurievb/OpenLID", filename="model.bin", cache_dir=None)
...
if __name__ == "__main__":
        ...
        df_test_data.to_csv('../Open_LID_Output/OPENLID_Output_CSV_' + str(args.seed)+'.csv', index=False)

        clean_df['predict_iso'], clean_df['predict_score'] = zip(*clean_df['sentence'].progress_apply(get_glotlid_prediction))
        # df_test_data.head()

        clean_df.to_csv('../Open_LID_Output/OPENLID_Output_CSV_Clean_Text_' + str(args.seed)+'.csv', index=False)
